...
sealed class Either <out L,  out R>{
    ...
    data class LeftFailure<out L>(val failure: L): Either<L, Nothing>()

    /** * Represents the right side of [Either] class which by convention is a "Success". */
    data class RightSuccess<out R>(val success: R): Either<Nothing, R>()

    /**
     * Returns true if this is a Right, false otherwise.
     * @see Right
     */
    val isRight get() = this is RightSuccess<R>

    /**
     * Returns true if this is a Left, false otherwise.
     * @see Left
     */
    val isLeft get() = this is LeftFailure<L>

    /**
     * Creates a Left type.
     * @see Left
     */
    fun <L> left(a: L) = Either.LeftFailure(a)

    /**
     * Creates a Left type.
     * @see Right
     */
    fun <R> right(b: R) = Either.RightSuccess(b)
    ...
    fun fold(fnL: (L) -> Any, fnR: (R) -> Any): Any =
        when (this) {
            is LeftFailure -> fnL(failure)
            is RightSuccess -> fnR(success)
        }
}
...
fun <T, L, R> Either<L, R>.flatMap(fn: (R) -> Either<L, T>): Either<L, T> =
    when (this) {
        is Either.LeftFailure -> Either.LeftFailure(failure)
        is Either.RightSuccess -> fn(success)
    }
...
fun <L, R> Either<L, R>.getOrElse(value: R): R =
    when (this) {
        is Either.LeftFailure -> value
        is Either.RightSuccess -> success
    }
...
fun <L, R> Either<L, R>.onFailure(fn: (failure: L) -> Unit): Either<L, R> =
    this.apply { if (this is Either.LeftFailure) fn(failure) }
...
fun <L, R> Either<L, R>.onSuccess(fn: (success: R) -> Unit): Either<L, R> =
    this.apply { if (this is Either.RightSuccess) fn(success) }
